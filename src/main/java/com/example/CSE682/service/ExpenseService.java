...
public class ExpenseService implements IExpenseService {
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getAll()
	 */
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getExpenseById(Long)
	 */
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#save(Expense)
	 */
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#edit(Expense, Long)
	 */
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#deleteById(Long)
	 */
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getTotalCost()
	 */
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getTotalCostCurrentMonthByCategoryId(Long)
	 */
	public double getTotalCostCurrentMonthByCategoryId(Long id) {
		...
		double count = expenseRepository.getCountByCategoryBetweenTwoDates(category.get(), first_of_month,
				last_of_month);
		if (count > 0) {
			double rtn = expenseRepository.getTotalCostBetweenTwoDatesByCategory(first_of_month, last_of_month,
					category.get(), userService.getLoggedinUser());
			return rtn;
		} else {
			return 0.0;
		}
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getTotalCostCurrentMonth()
	 */
	public double getTotalCostCurrentMonth() {
		LocalDate first_of_month = LocalDate.now().with(TemporalAdjusters.firstDayOfMonth());
		LocalDate last_of_month = LocalDate.now().with(TemporalAdjusters.lastDayOfMonth());
		if (expenseRepository.getCountBetweenTwoDates(first_of_month, last_of_month,
				userService.getLoggedinUser()) > 0) {
			return expenseRepository.getTotalCostBetweenTwoDates(first_of_month, last_of_month,
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getTotalCostCurrentMonthByCategory()
	 */
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getTimeseriesChart()
	 */
	public ArrayList<ArrayList<Object>> getTimeseriesChart() {
		...
		long monthsBetween = ChronoUnit.MONTHS.between(YearMonth.from(earliest_date),
				YearMonth.from(first_of_current_month));
		...
		for (int i = 0; i <= monthsBetween; i++) {
			...
			try {
				total_costs_per_month.add(
						expenseRepository.getTotalCostBetweenTwoDates(date, end_date, userService.getLoggedinUser()));
			} catch (Exception e) {
		...
		ArrayList<Category> categories = new ArrayList<Category>(
				categoryRepository.findAllByUser(userService.getLoggedinUser()));
		...
		for (Category c : categories) {
			...
			for (int i = 1; i < dates.size(); i++) {
				...
				try {
					double cost_this_month_for_this_category = expenseRepository.getTotalCostBetweenTwoDatesByCategory(
							curr_month_start, curr_month_end, c, userService.getLoggedinUser());
	...
	@Override
	/**
	 * @see com.example.CSE682.service.IExpenseService#getMonthlySummaryBar()
	 */
